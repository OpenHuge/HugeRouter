    ...
    state: {
      ...
      session: {
        ...
        user: {
          ...
          isPlatformAdmin: session.activeTenantId == null
        }
      }
    }
  }
...
function mapAuthResult(
  ...
  if (authenticatedState.kind !== 'authenticated') {
    throw new AuthApiError('HugeRouter auth callback did not create a session.', 500)
  }
...
function mapEmailCompletionResult(response: AuthLoginResult): EmailLoginCompleteResult {
  const authenticatedState = mapAuthenticatedSession(
    response.session,
    getDefaultProviderAvailability()
  ).state
  ...
  if (authenticatedState.kind !== 'authenticated') {
    throw new AuthApiError('HugeRouter email login did not create a session.', 500)
  }
...
function toAuthApiError(error: unknown): AuthApiError {
  if (error instanceof ControlPlaneClientError) {
    return new AuthApiError(error.message, error.status, error.code, error.meta)
  }
  ...
  return {
    async completeAuthCallback(
      ...
      try {
        ...
        const response = await client.completeOAuthLogin(oauthProvider, {
          code: payload.code ?? '',
          ...(payload.redirectTo ? { redirectUri: payload.redirectTo } : {}),
          state: payload.state ?? ''
        })
    ...
    async startProviderLogin(
      ...
      try {
        ...
        const response = await client.startOAuthLogin(provider as OAuthProvider, payload)
