  ...
  loader: async ({ deps: search }) => {
    ...
    if (search.error === "access_denied") {
      return createFailureView(
        ...
        search.errorDescription ??
          "The provider returned access_denied before HugeRouter could create a session.",
        "orange",
      );
    }
    ...
    if (!search.code && !search.state) {
      return createFailureView(
        ...
        search.errorDescription ??
          "The provider callback did not include a usable authorization payload.",
      );
    }
    ...
    try {
      ...
      if (result.data.outcome === "authenticated") {
        ...
        throwRedirect({
          to: getPostLoginDestination(
            result.data.state,
            result.data.redirectTo ?? search.redirect,
          ),
        });
      }
      ...
      <AuthStatusCard
        ...
        actions={
          <>
            ...
            <UiLink component={Link} c="dimmed" size="sm" to="/login">
              Start over
            </UiLink>
          </>
        }
