use super::{
    ...
    DELIVERY_CODE_TYPE_REDEMPTION, DeliveryCodeRecord, DeliveryRecord, PostgresStore,
    PreparedDeliveryRecords, StoreMode, delivery_projection, delivery_secret_key,
};
...
pub fn owner_redemption_capacity_blocker_for_memory(
    _deliveries: &[DeliveryRecord],
    _codes: &[DeliveryCodeRecord],
    _prepared: &PreparedDeliveryRecords,
) -> Option<super::DeliveryPrepareResult> {
    None
}
...
) -> Result<Option<super::DeliveryPrepareResult>> {
    ...
    PrepareDeliveryRedemptionUnitsResponse {
        data: DeliveryRedemptionUnitsBatch {
            ...
            capacity: u32::try_from(
                crate::delivery_redemption_policy::UNBOUNDED_ACTIVE_REDEMPTION_UNITS_PER_OWNER,
            )
            .unwrap_or(u32::MAX),
            units: units
                .iter()
