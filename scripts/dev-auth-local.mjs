...
children.push(
  startProcess('control-plane-api', 'cargo', ['run', '-p', 'control-plane-api', '--', 'serve'])
)
children.push(
...
try {
  await Promise.all([
    waitForUrl('control-plane-api', `${controlPlaneBaseUrl}/api/control-plane/auth/providers`),
    waitForUrl('console-web', `${consoleBaseUrl}/login`)
  ])
  console.log(`[open] Use ${consoleBaseUrl}/login for local OAuth callbacks.`)
  console.log('[note] Keep this process running until the OAuth login finishes.')
} catch (error) {
