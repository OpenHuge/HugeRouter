...
export const tradeOrderIdSchema = prefixedId("tradeord_");
...
export const aiProductRiskTierSchema = z.enum(["green", "yellow", "red"]);
...
export const escrowModeSchema = z.enum([
  "platform_ledger",
  "psp_escrow",
  "ton_contract",
]);
export const tradeOrderStateSchema = z.enum([
  "created",
  "escrow_funded",
...
export const trialConnectionStatusSchema = z.enum([
  "active",
  "paused",
  "needs_rotation",
]);
export const relayEvaluationRunnerModeSchema = z.enum(["simulated", "replay"]);
export const relayEvaluationVerdictSchema = z.enum([
  "healthy",
  "warning",
  "fail",
]);
export const relayCheckStatusSchema = z.enum([
  "pass",
  "warning",
  "fail",
  "not_tested",
...
export const merchantShopSchema = z.object({
  ...
  slug: slugSchema,
  display_name: z.string().min(1),
  seller_alias: z.string().min(1),
  identity_level: sellerIdentityLevelSchema,
  guarantee_deposit_usd: z.string().regex(/^\d+(\.\d+)?$/),
  dispute_rate_bps: z.number().int().nonnegative(),
...
export const cardProductSchema = z.object({
  ...
  risk_tier: aiProductRiskTierSchema,
  review_status: productReviewStatusSchema,
  escrow_mode: escrowModeSchema,
  required_kyc_level: sellerIdentityLevelSchema,
...
export const tradeOrderSchema = z.object({
  trade_order_id: tradeOrderIdSchema,
  tenant_id: tenantIdSchema,
  merchant_shop_id: merchantShopIdSchema,
  card_product_id: cardProductIdSchema,
  buyer_alias: z.string().min(1),
  seller_alias: z.string().min(1),
  state: tradeOrderStateSchema,
  escrow_mode: escrowModeSchema,
...
export type BalanceProjectionResponse = z.infer<
  typeof balanceProjectionResponseSchema
>;
export type PricingSimulationRequest = z.infer<
  typeof pricingSimulationRequestSchema
>;
export type PricingSimulationLineItem = z.infer<
  typeof pricingSimulationLineItemSchema
>;
export type PricingSimulationResponse = z.infer<
  typeof pricingSimulationResponseSchema
>;
...
export type BillingExportJobResponse = z.infer<
  typeof billingExportJobResponseSchema
>;
export type BillingExportJobsResponse = z.infer<
  typeof billingExportJobsResponseSchema
>;
...
>;
export type RouteReceiptDecisionTraceStep = z.infer<
  typeof routeReceiptDecisionTraceStepSchema
>;
export type RouteReceiptPolicyCheck = z.infer<
  typeof routeReceiptPolicyCheckSchema
>;
export type RouteReceiptProviderAttempt = z.infer<
  typeof routeReceiptProviderAttemptSchema
>;
export type RouteReceiptDiagnosticsResponse = z.infer<
  typeof routeReceiptDiagnosticsResponseSchema
>;
export type RouteDiagnosticDecision = z.infer<
  typeof routeDiagnosticDecisionSchema
>;
...
export type MerchantWorkspaceResponse = z.infer<
  typeof merchantWorkspaceResponseSchema
>;
